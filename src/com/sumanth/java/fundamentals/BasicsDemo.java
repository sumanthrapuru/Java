package com.sumanth.java.fundamentals;  //package com.semanticsquare.basics;
 ...
 class APIResponseParser {
	...
	public static Book parse(String response) {
		...
		String endRule = "<";
		int i = 0;
		String[] startRules = {"<title>", "<name>", "<original_publication_year>", "<average_rating>", "<ratings_count>", "<image_url>"};
		String[] result = new String[startRules.length];
		for(String str : startRules){
			result[i++] = parse(response, str, endRule);
		}
		i = 0;
		book.setTitle(result[i++]);
		book.setAuthor(result[i++]);
		book.setPublicationYear(Integer.parseInt(result[i++]));
		book.setAverageRating(Double.parseDouble(result[i++]));
		book.setRatingsCount(Integer.parseInt(result[i++].replaceAll(",", "")));
		book.setImageUrl(result[i++]);
	...
	public static String parse(String response, String startRule, String endRule){
		String[] strArrray = response.split(startRule);
		if(strArrray.length > 1){
			return  strArrray[1].split(endRule)[0];
		}else{
			return response.split(startRule.substring(0, startRule.length()-2))[1].split(">")[1].split(endRule)[0];
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(APIResponseParser.parse(response));;
	}
}